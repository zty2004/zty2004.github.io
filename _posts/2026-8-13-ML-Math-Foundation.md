...
- **Span:** $$\operatorname{span}(v_1,\dots,v_k) = \left\{ \sum_{i=1}^{k} c_i v_i : c_i \in \mathbb{R} \right\}$$ — the smallest subspace containing them.
...
$$(AB)_{ij} = \sum_{k} a_{ik} b_{kj}$$, which is why matrix product is
associative but not commutative — composing functions never is.
...
where $$a_j$$ are columns and $$\tilde{a}_i$$ rows. The **column view** says
...
- $$\operatorname{col}(A) = \{Ax\} \subseteq \mathbb{R}^m$$, $$\quad \ker(A) = \{x : Ax = 0\} \subseteq \mathbb{R}^n$$.
...
| Functional family | $$\{f_\theta\}_{\theta \in \Theta}$$ | the hypothesis class |
...
| $$f(X) = \mathrm{tr}(AX)$$ | $$\nabla_X f = A^\top$$ |
| $$f(X) = \log \det X$$ | $$\nabla_X f = X^{-\top}$$ |
| $$\sigma(z) = (1 + e^{-z})^{-1}$$ | $$\sigma' = \sigma(1 - \sigma)$$ |
...
as a stand-in for $$\mathcal{R}(\theta) = \mathbb{E}_{(x,y)}[\ell(f_\theta(x), y)]$$;
